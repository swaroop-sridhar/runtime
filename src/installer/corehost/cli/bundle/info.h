...
// bundle::info supports identification of a single-file app bundle, and
// minimap probing and mapping functionality only for the
// app.runtimeconfig.json and app.deps.json files.
//
...
namespace bundle
{
    struct info_t
    {
        struct config_t
        {
            ...
            {
                return m_location->isValid() && path.compare(m_path) == 0;
            }
