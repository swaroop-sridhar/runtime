...
bool deps_entry_t::to_path(const pal::string_t& base, bool look_in_base, pal::string_t* str) const
{
    ...
    if (exists)
    {
        trace::verbose(_X("    %s path query exists on disk %s"), query_type, candidate.c_str());
        return true;
    }
    else
    {
...
bool deps_entry_t::to_bundle_path(const pal::string_t& base, pal::string_t* str) const
{
    ...
    {
        trace::verbose(_X("    Base directory %s is different from bundle-base %s"), base, app->base_path());
        return false;
    }
